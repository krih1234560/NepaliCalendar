...
async function loadEvents() {
  ...
  try {
    ...
    if (holidayResponse.ok) {
      holidays = await holidayResponse.json();
    }

  } catch (error) {

    console.warn(
      "Festival / holiday data could not be loaded.",
      error
    );
...
function renderEvents() {
  ...
  const empty =
    document.getElementById("festivalEmpty");
  ...
  function render() {
    ...
    Object.entries(festivals || {}).forEach(
      ([date, items]) => {
...
function renderHolidays() {
  ...
  const filter =
    document.getElementById("holidayFilter");

  const empty =
    document.getElementById("holidayEmpty");
