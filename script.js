...
// Event data is imported directly from the JSON folder so the festival/holiday
// sections also work reliably after a Vite build, without depending on a
...
async function loadEvents() {
  // JSON files are bundled by Vite. Keep this function async so initialization
  // remains compatible with the rest of the page.
...
function eventMonthGroups(source, query, typeFilter = "all") {
  ...
  const all = Object.entries(source || {}).flatMap(([date, items]) => {
    ...
    return items.map(item => {
      ...
      return {
        ...
        monthData,
        dateText: monthData
          ? `${nepaliNumber(day)} ${monthData.np} ${nepaliNumber(year)}`
          : date
      };
    });
  });
  ...
  return all
    .filter(item => {
      ...
      // Search the actual JSON fields plus the formatted BS date/month.
      // This lets users search Nepali names, English names, YYYY-MM-DD,
      // Nepali month names, AD dates and holiday types.
      const searchText = [
        item.name,
        item.nameEn,
        item.date,
  ...
  return `
    <article class="festival-card" data-type="festival">
      <div class="festival-card-top">
        ...
        <div class="festival-date">${item.dateText}</div>
      </div>

      <div class="festival-content">
        <span class="festival-type">Festival</span>
        <h3>${item.name || ""}</h3>
        ${
          item.nameEn
            ? `<p>${item.nameEn}</p>`
  ...
  return `
    ...
    >
      <div class="holiday-card-top">
        ...
        <div class="holiday-date">${item.dateText}</div>
      </div>

      <span class="holiday-type">${typeLabel}</span>
      ...
      ${
        item.ad
          ? `<small class="holiday-ad">AD: ${item.ad}</small>`
          : ""
      }
    </article>
  `;
